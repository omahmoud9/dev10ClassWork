...
public class App {
    ...
    public static void main(String[] args) {
        ...
        while (isPlaying) {
            ...
            if (!result.isSuccess()) { // simplify always has the message
                if (game.isOver()) {
                    System.out.println(result.getMessage());
                } else if (result.getMessage().equals("Stone is off the board.")) { // simplify
                    System.out.println(result.getMessage());
            ...
            } else if (result.isSuccess()) {  // becomes else
            ...
            if (game.isOver()) {
                if (playAgain().equals("y")) {
                    isPlaying = true;
                    game = new Gomoku(player1,player2); // pass new players
                }else {
                    isPlaying = false;
                }
            }
